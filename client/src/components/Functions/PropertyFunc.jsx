...
import { Button, Loading, Input } from "@web3uikit/core";
import { useMoralisQuery, useMoralis } from "react-moralis";
import "../../styling/Home/Home.css";
import Navbar from "../Navbar";
import { ipfs, _ipfs } from "../../modules/ipfs"
...
const PropertyFunc = () => {
    ...
    try {
      ...
      for (let i = 1; i <= counter; i++) {
        ...
        if (propertyExists) {
          ...
          propertyObj.images = await getLinks(propertyObj.ipfsHash);
  ...
  //directory IPFS hash (stored in contract)
  async function getLinks(cid) {
    const links = [];
    for await (const link of _ipfs.ls(cid)) {
      links.push("https://ipfs.io/ipfs/" + link.path);
    }
    return links;
  }

  // adding a property
  const addProperty = async (addr) => {
    try {
      if (!checkAddress(addr)) {
        return;
      }
      ...
      var ipfsHash = "";
      for await (const result of ipfs.addAll(propertyImages, options)) {
        ipfsHash = result.cid._baseCache.entries().next().value[1];
      }
      //image
    ...
    <div className="App">
      ...
      <div className="hero-section">
        ...
        <Button
          ...
          onClick={() => addProperty(propertyLandlordAddress)}
        />
        {/* -------------------- */}
        {/* Modify a Property  */}
        <h2
          ...
          return (
            <Descriptions title="Property" bordered>
              ...
              <Descriptions.Item label="Image link" span={3}>
                {property.images.map(img => <img src={img} 
                style= {{
                  height: "100px",
                  width: "100px"
                }}
