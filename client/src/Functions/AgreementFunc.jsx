...
const AgreementFunc = () => {
  ...
  const landlordSignAgreement = async (landAddr, buyerAddr, propId) => {
    try {
      if (!window.ethereum) {
        message.error(
          "No wallet detected! Please install metamask or any other wallet to continue."
        );
      }
      if (!checkAddress(landAddr) || !checkAddress(buyerAddr)) {
        return;
      ...
      if (propId === "0" || parseInt(propId) < 0) {
        message.error(
          "Invalid ID! Property Id cannot be less than or equal to zero."
        );
        return;
      }
      const landlordExists = await checkLandlordExists(landAddr);
      ...
      const signatureMessage = String(
        landAddr.slice(0, 15) + buyerAddr.slice(0, 16) + propId.slice(0, 1)
      );
      // getting the signature of the landlord
  ...
  const buyerSignAgreement = async (landAddr, buyerAddr, propId) => {
    try {
      ...
      const signer = provider.getSigner();
      const address = await signer.getAddress();
      if (address !== buyerAddr) {
        message.error(
          "You cannot sign the transaction. Only the buyer can sign the transaction."
        );
        return;
      }
      const signatureMessage = String(
      ...
      // getting the signature of the landlord
      const buyerSignature = await signer.signMessage(signatureMessage);
      await realEstateContract.buyerSignedAgreement(
        landAddr,
        buyerAddr,
        parseInt(propId),
        buyerSignature
  ...
  const completeAgreementUsingContract = async (
    landAddr,
    buyerAddr,
    propId
  ) => {
    try {
      ...
      if (agreementStatus !== "2") {
        message.error("Agreement Draft cannot be completed. ");
        return;
      }
      await realEstateContract.completeAgreement(
        landAddr,
        buyerAddr,
        parseInt(propId)
      );
      message.success("Agreement Completed successfully. Congratulations!");
    } catch (error) {
      message.error("Error: " + error);
      return;
    }
    ...
    <div className="App">
      ...
      <div className="hero-section">
        ...
        <Button
          ...
          className="borderSquare"
          onClick={() => {
            completeAgreementUsingContract(
              landlordAddressComplete,
